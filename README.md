...
生产 `cua_skill_bytesso` 将本仓库作为唯一可信的 Credential Skill 源，按
已经 review 的完整 40 位 Git commit 从 GitHub codeload 安装到版本化私有
缓存。消费者拒绝任意仓库、分支名、tag、prompt 提供的 ref、目录穿越和
符号链接，并在执行前验证必需脚本与 `cua-target/v1` 契约。

Credential Skill 源码不需要单独的 TOS release manifest 或 Ed25519 发布
密钥。Credential Agent binary 和浏览器扩展仍使用各自独立的签名制品发布
链路，不能因为 Skill 使用 GitHub 分发而放宽。
...
Skill 会自行识别环境、定位 Agent、选择正确命令，并只在 OAuth、设备配对、敏感凭据输入或浏览器必须要求用户手势时让用户介入。
...
1. 检测操作系统、架构、Chrome/Chromium 和 Agent 安装状态。
2. 下载最新签名发布清单。
...
9. 安装或连接 AL Credential Center Chrome/Chromium 扩展。
10. Agent 把 Vault 当前启用的动态站点策略下发给扩展，并启用这些网站。
...
4. Agent 显示短期配对码。
5. 向用户展示待配对设备并取得明确批准；批准后在已经登录的个人电脑上执行：

   ```text
   credential-agent pair --approve --output json PAIR-CODE
   ```

   用户拒绝时使用 `--deny`；旧 Agent 不支持这些参数时才回退到交互式 `credential-agent pair PAIR-CODE`。

6. 云电脑完成设备入网并启动后台同步。
7. 准备 Chrome/Chromium 扩展；云电脑首次接收某个站点时随任务获得对应动态策略并完成精确权限授权。
8. 运行 `credential-agent doctor --strict`。
...
Agent 已经向用户展示并确认精确目标和网站后，自动编排使用机器输出：
...
`pending_target` 不是失败，也不是重新运行 `browser sync` 的依据。保留返回的 Job ID，优先使用 `job wait` 获得权威终态。
