...
- Options page reports HTTPS capability unavailable: inspect Chrome's Site access setting. Do not click an enable-all control or invoke a permission request; the page is read-only.
...
- `SITE_PERMISSION_REQUIRED` / `HOST_PERMISSION_REQUIRED`: Chrome is withholding the required HTTPS capability for the exact signed Policy origin. Restore Site access in Chrome and let the same heartbeat/Job continue; do not use `open-permissions`, `permissions.request()`, or a per-origin fallback.
...
- `TARGET_NETWORK_ASSIST_UNAVAILABLE` / `TARGET_NETWORK_UNREACHABLE`: these are degraded Connector observations after an authoritative Job already exists, not delivery results. Keep waiting for that exact Job. A later Job success wins; if it remains `waiting_network`, repair only the target environment's managed network path and resume observation without repeating Capture, Restore, Delivery Grant issuance, or `browser sync`.
...
- For browser jobs in `waiting_permission`, inspect target Site access read-only while leaving the original Job active. Do not open Options or create a second Job.
